...
btnSave.addEventListener("click", async ()=>{
  ...
  try{
    if (window.VLT_SYNC?.pushOnce) await window.VLT_SYNC.pushOnce("quiz_save");
  }catch(e){
...
btnSync.addEventListener("click", async ()=>{
  try{
    if (window.VLT_SYNC){
      ...
      await window.VLT_SYNC.pullOnce("quiz_manual_sync");
    }
...
(async ()=>{
  ...
  if (window.VLT_SYNC){
    window.VLT_SYNC.start();
    await window.VLT_SYNC.pullOnce("quiz_init");
  }
})();
